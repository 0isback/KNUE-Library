...
if login_response.status_code == 200:  # 로그인 성공 시
    ...
    if ticket_response.status_code == 200:
        ...
        print("My Ticket 응답 데이터:", ticket_data)

        # id 값 추출
        if ticket_data['success'] and ticket_data['data']['totalCount'] > 0:
            ticket_id = ticket_data['data']['list'][0]['id']  # 첫 번째 티켓의 id 값
            print(f"추출된 티켓 ID: {ticket_id}")

            # 네 번째 단계: confirm 요청
            confirm_url = f'https://eris.knue.ac.kr/smufu-api/api/mo/seat-charge/{ticket_id}/confirm'
            confirm_response = requests.post(confirm_url, headers=headers)

            # confirm 요청 결과 확인
            if confirm_response.status_code == 200:
                print("Confirm 응답 데이터:", confirm_response.json())
            else:
                print(f"Confirm 요청 실패. 상태 코드: {confirm_response.status_code}")
        else:
            print("유효한 티켓이 없습니다.")
    else:
        print(f"My Ticket 요청 실패. 상태 코드: {ticket_response.status_code}")
else:
